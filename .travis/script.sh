...
if [[ "$TEST" == "performance" ]]; then
  ...
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance || show_logs_and_return_non_zero
  else
    pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance.test_$PERFORMANCE_TEST || show_logs_and_return_non_zero
  fi
  exit
fi
