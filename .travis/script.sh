...
if [ "$TEST" = 'bindings' ]; then
  ...
  ./generate.sh pulp_maven ruby 0

  cd pulp_maven-client
  gem build pulp_maven_client
  gem install --both ./pulp_maven_client-0.gem
  cd ..
...
# Run unit tests.
cmd_prefix bash -c "PULP_DATABASES__default__USER=postgres django-admin test --noinput /usr/local/lib/python${TRAVIS_PYTHON_VERSION}/site-packages/pulp_maven/tests/unit/"
...
if [[ "$TEST" == "performance" ]]; then
  ...
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance
  else
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance.test_$PERFORMANCE_TEST
  fi
  exit
fi
