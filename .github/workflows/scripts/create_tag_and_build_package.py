...
helper = textwrap.dedent(
    """\
        ...
        and saved as pulp-maven-.tar.gz and pulp_maven-[tag]-py3-none-any.whl in the
        'dist' directory.

        If the package does not exist on PyPI, two new packages are built with the names
        pulp-maven-[tag].tar.gz and pulp_maven-[tag]-py3-none-any.whl. They are
...
package_found = asyncio.run(get_package_from_pypi("pulp-maven=={tag.name}", plugin_path))
