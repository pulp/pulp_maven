...
then
  ...
  then
    ...
    for item in $(jq -r '.versions[]| select(.component!="maven")| tojson' <<<"$REPORTED_STATUS")
    do
      echo $item
      COMPONENT="$(jq -r '.component' <<<"$item")"
      VERSION="$(jq -r '.version' <<<"$item")"
  ...
  then
    ...
    # Sadly: Different pulpcore-versions aren't either...
    for item in $(echo "$REPORTED_STATUS" | jq -r '.versions[]|select(.component!="maven")|(.package // ("pulp_" + .component)|sub("pulp_core"; "pulpcore"))|sub("-"; "_")')
    do
      ./generate.sh "${item}" python
      cmd_prefix pip3 install "/root/pulp-openapi-generator/${item}-client"
      sudo rm -rf "./${item}-client"
