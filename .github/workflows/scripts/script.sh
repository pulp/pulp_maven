...
if [[ "$TEST" = "docs" ]]; then
  if [[ "$GITHUB_WORKFLOW" == "Maven CI" ]]; then
    pip install towncrier==19.9.0
    towncrier --yes --version 4.0.0.ci
  fi
  cmd_prefix bash -c "cd pulp_maven/docs; make diagrams html; tar -cvf docs.tar ./_build"
  docker cp pulp:/pulp_maven//docs/docs.tar docs.tar
...
if [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
  ...
  REPORTED_VERSION=$(http $STATUS_ENDPOINT | jq --arg plugin maven --arg legacy_plugin pulp_maven -r '.versions[] | select(.component == $plugin or .component == $legacy_plugin) | .version')
  response=$(curl --write-out %{http_code} --silent --output /dev/null https://pypi.org/project/pulp-maven/$REPORTED_VERSION/)
  if [ "$response" == "200" ];
  then
    echo "pulp_maven $REPORTED_VERSION has already been released. Skipping running tests."
    exit
  fi
fi
...
if [[ "$TEST" == "plugin-from-pypi" ]]; then
  ...
  git checkout ${COMPONENT_VERSION} -- pulp_maven/tests/
fi
...
./generate.sh pulp_maven python
cmd_prefix pip3 install /root/pulp-openapi-generator/pulp_maven-client
sudo rm -rf ./pulp_maven-client
...
# Run unit tests.
cmd_prefix bash -c "PULP_DATABASES__default__USER=postgres pytest -v -r sx --color=yes -p no:pulpcore --pyargs pulp_maven.tests.unit"

# Run functional tests
if [[ "$TEST" == "performance" ]]; then
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance"
  else
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_maven.tests.performance.test_$PERFORMANCE_TEST"
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    if [[ "$GITHUB_WORKFLOW" == "Maven Nightly CI/CD" ]] || [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_maven.tests.functional -m parallel -n 8 --nightly"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_maven.tests.functional -m 'not parallel' --nightly"
        ...
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_maven.tests.functional -m parallel -n 8"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_maven.tests.functional -m 'not parallel'"
