...
then
  ...
  docker exec pulp pip3 install pulp-maven-client==$VERSION
  mkdir -p dist
...
find . -name "*.whl" -exec docker exec pulp pip3 install /root/pulp-openapi-generator/pulp_maven-client/{} \;
...
tar cvf ../../pulp_maven/python-client-docs.tar ./docs
exit $?
