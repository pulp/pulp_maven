...
`5.3.17.rhlw-00001`, `5.3.17.rhlw-00001-n0001`, and `5.3.18.lw-1` share bases `5.3.17` / `5.3.18`. Versions that are not that last-segment shape are unchanged: `4.3.0-redhat-1`, `5.3.18-anything`, `5.3.18.anything`. `5.3.180` is a different version than `5.3.18`.
...
Omit `collapse_builds`. Filter with `group_id`, `artifact_id`, and `base_version` so `5.3.17` also hits `5.3.17.rhlw-00001` and `5.3.17.rhlw-00001-n0001`, and does **not** match `5.3.170`. Clients map the flat GAV list to nested `builds[]`.
