...
def _uid():
    # Unique groupIds so parallel workers do not collide on MavenArtifact GAV.
    # Artifact sha256 uniqueness is handled by pom_file_factory (per-file salt).
...
def test_predisclosure_rebuild_suffix(
    ...
    repo = _upload_gavs(
        ...
        monitor_task,
        [
            (group_id, "hello", "5.3.17"),
            (group_id, "hello", "5.3.17.rhlw-00001"),
            (group_id, "hello", "5.3.17.rhlw-00001-n0001"),
        ],
    )
    data = _api_get(bindings_cfg, f"{repo.pulp_href}packages/")
    assert data["count"] == 1
    ...
    assert {item["version"] for item in matched["results"]} == {
        "5.3.17",
        "5.3.17.rhlw-00001",
